...
<div class="container mt-4">
    <div class="edit-form-container">
        <h3 class="text-center mb-4">Chỉnh sửa thông tin Admin</h3>
        ...
        <form action="{{ route('update.admin', $edit_admin->admin_id) }}" method="POST">
                ...
                <select name="role_id" class="form-control form-control-sm" required>
                    ...
                    @foreach($roles as $role)
                        <option value="{{ $role->role_id }}" 
                            {{ (old('role_id', $edit_admin->role_id) == $role->role_id) ? 'selected' : '' }}>
                            {{ $role->role_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="text-center mt-4">
                <button onclick="return confirm('Xác nhận cập nhật thông tin?')" type="submit" class="btn btn-success btn-sm px-4">Lưu thay đổi</button>
